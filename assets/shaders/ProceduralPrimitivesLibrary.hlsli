...
bool RayAnalyticGeometryIntersectionTest(in Ray ray, in PrimitiveType::Enum analyticPrimitive, out float thit, out ProceduralPrimitiveAttributes attr)
{
    float3 aabb[2] = {
        float3(-1,-1,-1),
        float3(1,1,1)
    ...
    {
    case PrimitiveType::AnalyticPrimitive_AABB: return RayAABBIntersectionTest(ray, aabb, thit, attr);
    case PrimitiveType::AnalyticPrimitive_Spheres: return RaySpheresIntersectionTest(ray, thit, attr);
...
// Analytic geometry intersection test.
// AABB local space dimensions: <-1,1>.
bool RayVolumetricGeometryIntersectionTest(in Ray ray, in PrimitiveType::Enum volumetricPrimitive, out float thit, out ProceduralPrimitiveAttributes attr, in float elapsedTime)
{
    switch (volumetricPrimitive)
    {
    case PrimitiveType::VolumetricPrimitive_Metaballs: return RayMetaballsIntersectionTest(ray, thit, attr, elapsedTime);
    default: return false;
    }
}
...
float GetDistanceFromSignedDistancePrimitive(in float3 position, in PrimitiveType::Enum signedDistancePrimitive)
{
